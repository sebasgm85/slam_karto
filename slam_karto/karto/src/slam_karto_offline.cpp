...
/* Author: Brian Gerkey, Stephan Wirth */
...
class SlamKarto
{
  public:
    ...
    // reads all parameters from node handle and sets up subscribers and publishers
    void initNode();
...
public:
  ...
  ~KartoMapper()
  {
    std::cout << "Mapper processed " << processed_scans_ << " of " 
      << num_scans_ << " laser scans." << std::endl;
  }

  /**
   * Converts multi echo laser scans to normal single echo scans and calls
   * laserCallback().
   */
  void multiEchoLaserCallback(
      const sensor_msgs::MultiEchoLaserScan::ConstPtr& mel_scan_msg)
  {
    sensor_msgs::LaserScan::ConstPtr scan_msg = laser_proc::LaserProc::getFirstScan(*mel_scan_msg);
    laserCallback(scan_msg);
  }

  /**
   * Retrieves the corresponding laser scanner pose for given scan message
   * and calls addScan()
   */
  void laserCallback(
  ...
  {
    karto::Pose2 karto_pose;
    if (!getPose(scan_msg->header.stamp, scan_msg->header.frame_id, "/odom", karto_pose))
    {
      ROS_DEBUG("Skipping scan.");
      return;
    }
    addScan(*scan_msg, karto_pose);
  }

  /**
   * Retrieve pose of given frame_id with respect to given reference_frame
   * at given time stamp using transformer_.
   */
  bool getPose(const ros::Time& stamp, const std::string& frame_id, 
      const std::string& reference_frame, karto::Pose2& karto_pose)
  {
    ROS_ASSERT(transformer_.get() != NULL);

    tf::Transform identity; 
    identity.setIdentity();
    tf::Stamped<tf::Pose> ident(identity, stamp, frame_id);
    tf::Stamped<tf::Pose> pose;
    try
    {
      transformer_->transformPose(reference_frame, ident, pose);
    }
    catch(tf::TransformException e)
    {
      ROS_WARN("Failed to compute pose: %s", e.what());
      return false;
    }
  ...
  /**
   * Sets the transformer that is used in getPose()
   */
  void setTransformer(const boost::shared_ptr<tf::Transformer>& transformer)
  {
  ...
  void saveMap(const std::string& filename)
  {
    ...
    karto::OccupancyGrid* occ_grid = 
            karto::OccupancyGrid::CreateFromScans(mapper_.GetAllProcessedScans(), resolution);
...
private:
  ...
  /**
   * Create a Karto laser range finder with properties from given scan
   * msg.
   */
  karto::LaserRangeFinder* createLaserRangeFinder(
      const sensor_msgs::LaserScan& scan_msg)
  {
    std::string name = scan_msg.header.frame_id;
    karto::LaserRangeFinder* laser = 
      karto::LaserRangeFinder::CreateLaserRangeFinder(
          karto::LaserRangeFinder_Custom, karto::Name(name));
    laser->SetOffsetPose(karto::Pose2(0.0, 0.0, 0.0));
    laser->SetMinimumRange(scan_msg.range_min);
    laser->SetMaximumRange(scan_msg.range_max);
    laser->SetMinimumAngle(scan_msg.angle_min);
    laser->SetMaximumAngle(scan_msg.angle_max);
    laser->SetAngularResolution(scan_msg.angle_increment);
    // TODO make this a param
    laser->SetRangeThreshold(scan_msg.range_max);
      
    // Add it to the dataset, which makes it available for the sensor manager
    dataset_.Add(laser);
    return laser;
  }
      ...
      const karto::Pose2& karto_pose)
  {
    ...
    if(!laser_range_finder_)
    {
      // create laser on first run
      laser_range_finder_ = createLaserRangeFinder(scan_msg);

      if (!laser_range_finder_)
      {
        ROS_ERROR("Failed to create laser device for %s.",
          scan_msg.header.frame_id.c_str());
        return;
      }
    }
 
    std::vector<kt_double> readings(scan_msg.ranges.begin(), scan_msg.ranges.end());
  
    // create localized range scan
    karto::LocalizedRangeScan* range_scan = 
      new karto::LocalizedRangeScan(laser_range_finder_->GetName(), readings);
    range_scan->SetOdometricPose(karto_pose);
    range_scan->SetCorrectedPose(karto_pose);
  ...
  karto::LaserRangeFinder* laser_range_finder_;

  // number of scans given to addScan
  int num_scans_;
  // number of scans accepted by the mapper
  int processed_scans_;
...
int main(int argc, char** argv)
{
  if (argc < 3)
  {
    std::cout << "Usage:\n\t" << argv[0] << " BAGFILE [BAGFILE...] OUT_MAP_FILE" << std::endl;
    return 0;
  }
  //ros::init(argc, argv, "slam_karto");
  BagProcessor<sensor_msgs::MultiEchoLaserScan> processor("/hokuyo/echoes");
  KartoMapper mapper;
  processor.registerCallback(boost::bind(&KartoMapper::multiEchoLaserCallback, &mapper, _1));
  mapper.setTransformer(processor.getTransformer());
  ...
  std::string map_filename(argv[argc-1]);
  std::cout << "Saving map to " << map_filename << "..." << std::flush;
  mapper.saveMap(map_filename);
