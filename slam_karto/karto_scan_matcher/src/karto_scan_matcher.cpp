                                   ...
                                   const DoubleVector& search_sizes, const DoubleVector& search_resolutions)
{
  ...
  laser_to_base_ = tf::Transform(tf::createQuaternionFromYaw(laser_pose.theta).asBt(),
                               tf::Vector3(laser_pose.x, laser_pose.y, 0.0));
  string suffix;
  {
...
{
  tf::Transform laser_to_base(tf::createQuaternionFromYaw(offset.GetHeading()).asBt(),
                            tf::Vector3(offset.GetX(), offset.GetY(), 0.0));
  tf::Transform laser_to_map(tf::createQuaternionFromYaw(pose.GetHeading()).asBt(),
                           tf::Vector3(pose.GetX(), pose.GetY(), 0.0));
  tf::Transform base_to_map = laser_to_map*laser_to_base.inverse();
  gm::Pose2D result;
...
void addPoints (PointCloud* cloud, const gm::Pose2D& pose,
                ...
                const tf::Transform& laser_to_base)
{
  ...
  BOOST_FOREACH (const double range, scan.ranges)
  {
    if ((scan.range_min < range) && (range < scan.range_max))
    {
      ...
      tf::Vector3 p(range*cos(theta), range*sin(theta), 0);
      tf::Vector3 p2 = laser_to_base*p;
      tf::Vector3 p3 = tf::Transform(tf::createQuaternionFromYaw(pose.theta),
                                 tf::Vector3(pose.x, pose.y, 0))*p2;
      pcl::PointXYZRGB pt;
      pt.x = p3.x();
      pt.y = p3.y();
      pt.z = p3.z();
